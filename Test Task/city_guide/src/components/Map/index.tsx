    ...
    for (let i = 0; i < geoObjects.selectedCategories.length; i++) {
      try {
        ...
        const response = await fetch(
          `https://search-maps.yandex.ru/v1/?text=${geoObjects.selectedCategories[i].text}&type=biz&lang=ru_RU&apikey=${API_KEY2}&rspn=1&ll=${userLocation[1]},${userLocation[0]}&spn=${radius},${radius}&results=100`
        );
